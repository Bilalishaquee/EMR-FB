...
router.post('/', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // If user is a doctor, automatically assign patient to them
    if (req.user.role === 'doctor') {
...
router.post('/:id/visits/initial', async (req, res) => {
  try {
    const patient = await Patient.findById(req.params.id);
    ...
    res.status(201).json({
      message: 'Initial visit created successfully',
      visit
    });
  } catch (error) {
...
router.post('/:id/visits/followup', async (req, res) => {
  try {
    const patient = await Patient.findById(req.params.id);
...
router.post('/:id/visits/discharge', async (req, res) => {
  try {
    const patient = await Patient.findById(req.params.id);
    ...
    res.status(201).json({
      message: 'Discharge visit created successfully',
      visit
    });
  } catch (error) {
...
router.get('/visits/:visitId', async (req, res) => {
  try {
    ...
    // If user is a doctor, check if visit is for their patient
